...
describe("fold", () => {
  ...
  it("Multiple Named", () => {
    const result = treeFold(seq(new Named("a", text("1")), new Named("b", text("2"))));
  ...
  it("Named accumulates", () => {
    const result = treeFold(seq(new Named("x", text("a")), new Named("x", text("b"))));
    const m = result as Map<string, TreeValue>;
  ...
  it("NamedAsList accumulates", () => {
    const result = treeFold(seq(new NamedAsList("items", text("a")), new NamedAsList("items", text("b"))));
    const m = result as Map<string, TreeValue>;
  ...
  it("Override wins", () => {
    const result = treeFold(seq(new Named("x", text("ignored")), text("also ignored"), new Override(text("result"))));
    expect(result).toBe("result");
  });

  it("Nested Named", () => {
    const result = treeFold(new Named("x", seq(new Named("a", text("1")), new Named("b", text("2")))));
    const m = result as Map<string, TreeValue>;

...
export function compile(grammar: string, cfg?: Cfg): Grammar {
  try {
    ...
    throw new ApiError(
      `grammar compilation failed: ${(e as Error).message}`,
      e,
    );
  }
}
...
export function loadGrammarFromJSON(json: string, _cfg?: Cfg): Grammar {
  const data = JSON.parse(json);
  return loadJSON(data);
}

export function grammarToJSON(_grammar: string, _cfg?: Cfg): string {
  throw new ApiError(
    "grammarToJSON is not yet implemented — needs Grammar.toJSON() serialization.",
  );
}

export function grammarPretty(_grammar: string, _cfg?: Cfg): string {
  throw new ApiError(
...
export async function loadGrammar(
  ...
  if (e === "json") {
    ...
    return loadGrammarFromJSON(raw, cfg);
  }
